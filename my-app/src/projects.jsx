      ...
      <div className="max-w-6xl mx-auto">
        <div className="mb-16">
          ...
          <p className="text-lg text-slate-600 mt-4 max-w-2xl">
            A selection of my recent work, showcasing my skills in software engineering and web development.
          </p>
        </div>
        ...
        {isMobile ? (
          <div className="relative pb-12">
              ...
              <div className="p-6 pt-0">
                <div className="flex flex-wrap gap-2">
                  {projectsData[currentIndex].tech.map((techName) => (
                    <span
                      key={techName}
                      className="inline-flex items-center rounded-full bg-secondary/80 text-primary px-3 py-1 text-xs font-semibold border border-primary/10"
                    >
                      {techName}
                    </span>
                  ))}
            ...
            <div className="absolute bottom-0 left-0 right-0 flex justify-center gap-4">
              <button
                onClick={prevProject}
                className="bg-white hover:bg-slate-50 text-slate-700 rounded-full p-3 shadow-md hover:shadow-lg border border-slate-100 transition-all focus:outline-none focus:ring-2 focus:ring-primary"
                aria-label="Previous"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-2 gap-10">
            {projectsData.map(({ title, description, image, tech, githubLink, liveDemoLink, status }) => (
              <div
                key={title}
                className="rounded-2xl border border-slate-100 bg-white flex flex-col h-full overflow-hidden shadow-lg shadow-slate-200/40 hover:shadow-xl hover:-translate-y-1 transition-all duration-300"
              >
                <div className="aspect-video relative overflow-hidden group">
                  <img
                    alt={title}
                ...
                <div className="p-8 pt-0">
                  <div className="flex flex-wrap gap-2">
                    {tech.map((techName) => (
                      <span
                        key={techName}
                        className="inline-flex items-center rounded-full bg-secondary text-primary px-3 py-1.5 text-xs font-semibold border border-primary/10"
                      >
                        {techName}
                      </span>
                    ))}
